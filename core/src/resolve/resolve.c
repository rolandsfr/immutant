...
int resolve_keyword(char* line, size_t pos, size_t len, enum TokenType* token_out)
{
    char* lexeme = line + pos;

    // check first letter first to reduce number of character comparisons later caused by strcmp
    // additionally short circuit on length check
    switch (lexeme[0]) { 
        case 'e':
